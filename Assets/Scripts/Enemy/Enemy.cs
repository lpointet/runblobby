...
public class Enemy : Character {
	...
	[SerializeField] protected GameObject _startGround;
	[SerializeField] protected GameObject _arenaGround;
	...
	public GameObject startGround {
		get { return _startGround; }
	}
	public GameObject arenaGround {
		get { return _arenaGround; }
	}
	public GameObject endGround {
		get { return _endGround; }
	}
	...
	protected override void Update () {
		...
		if (TimeManager.time > timeToFire) {
			timeToFire = TimeManager.time + delayBetweenAtttack * Random.Range(3 / 4f, 5 / 4f);

			// Calcul de la nouvelle attaque
			int newAttackNumber = attackList[Random.Range(0, attackList.Count)];

			ChooseAttack (newAttackNumber);

			if (numberOfAttack > 1) { // On n'enlève et ne remet les attaques que s'il y en a au moins 2
				if (previousAttackNumber > 0) // Si on a déjà eu une attaque avant, on peut rajouter celle d'encore avant
				attackList.Add (previousAttackNumber);

				attackList.Remove (newAttackNumber);

				previousAttackNumber = newAttackNumber;
			}
		}
	...
	protected virtual void ChooseAttack (int attackNumber) {
